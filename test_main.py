...
def test_create_account(name):
    url = ACCOUNT_API

    headers = {'accept': 'application/json', 'Content-Type': 'application/json'}
    data = {"account": name, "email": f"{name}@mail.com", "password": TEST_PASSWORD}

    response = client.post(url, headers=headers, json=data)
    expected_response = {"account": name}
...
def verify_create_order(name, data):
    resp = verify_login(name)
...
def verify_get_order(headers, id: int, data):
    url = f"{ORDER_API}{id}"
...
@pytest.mark.parametrize("data", [{"order_type": "Sell", "symbol": "USDJPY", "volume": 4000}])
def test_delete_order(name, data):
    headers, id = verify_create_order(name, data)
    verify_get_order(headers, id, data)
